...
const CategorySelectionScreen = () => {
  const [selectedCategories, setSelectedCategories] = useState<{ name: string }[]>([]);
  ...
  const handleProceed = async () => {
    ...
    try {
      if (selectedCategories.length >= 5) {
        ...
        const res = await api.handleCourseCategorySelection(body.selectedCategories, userToken);
      ...
      showAlert('error', 'An error occurred while proceeding. Please try again.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <View style={styles.categoryGrid}>
            {categories.map((category) => (
              <TouchableOpacity
                ...
                style={[
                  styles.categoryItem,
                  selectedCategories.some(item => item.name === category.name) && styles.selectedCategory,
                ]}
                onPress={() => toggleCategory(category.name)}
              >
                <Text style={styles.categoryIcon}>{category.icon}</Text>
                <Text style={styles.categoryName}>{category.name}</Text>
                {selectedCategories.some(item => item.name === category.name) && (
